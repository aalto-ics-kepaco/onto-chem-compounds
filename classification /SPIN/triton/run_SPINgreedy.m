...
function run_SPINgreedy(c,g,nb,penalty,type,filename)
    ...
    if strcmp(filename(1:4),'meme')
        ...
        X=dlmread(sprintf('../testing/%s.BabPub.phix',filename));
        Y=dlmread(sprintf('../testing/%s.Y',filename));
        Yp=Y;
        Y(Y>0)=1;
        E=dlmread(sprintf('../testing/%s.e',filename));
        E=E(:,1:2);
        Yv=dlmread(sprintf('../testing/%s.Yv',filename));
        YvSum=sum(Yv,2);
        Yv=Yv(YvSum>0,:);
        suffix=sprintf('%s_%s_%s_%s_%s_%s_SPINgreedy', filename,c,g,nb,penalty,type)
    ...
    %   X=dlmread(sprintf('../onto/%s.babel.phix',filename));
        X=dlmread(sprintf('../onto/%s.babPubc.phix',filename)); 
   ...
   %% change Y from -1 to 0: labeling (0/1), for the sake of base learner svm, will change back in later section
    ...
    % N fold cross validation
     for k=1:nfold
        ...
        %for triton
        load(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/tmp/Ypred_%s.mat', paramsIn.filestem));
   ...
   %dlmwrite(sprintf('/home/ilievsm1/T_%s.mat',paramsIn.filestem),running_times);
        filename=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/results_GgreedyLab_updated/Ypred_%s.mat',paramsIn.filestem));
	save(filename,'YpredVal');
	fname=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/results_GgreedyLab_updated/T_%s.mat',paramsIn.filestem));
