...
function run_SPINonto(c,g,nb,penalty,type,filename)
    ...
    if strcmp(filename(1:4),'meme')
        ...
        X=dlmread(sprintf('/triton/ics/work/ilievsm1/for_triton/onto/%s.babPubc.phix',filename)); 
    %   Y=dlmread(sprintf('../dblp/dblp.inp.%s.lda.Y',strrep(filename,'_','.')));
        Y=dlmread(sprintf('/triton/ics/work/ilievsm1/for_triton/onto/%s.Y',filename));
        Yp=Y;
        Y(Y>0)=1;
    %   E=dlmread(sprintf('../dblp/dblp.inp.%s.e',strrep(filename,'_','.')));
        E=dlmread(sprintf('/triton/ics/work/ilievsm1/for_triton/onto/%s.e',filename));
        E=E(:,1:2);
    %   Yv=dlmread(sprintf('../dblp/dblp.inp.%s.lda.Yv',strrep(filename,'_','.')));
        Yv=dlmread(sprintf('/triton/ics/work/ilievsm1/for_triton/onto/%s.Yv',filename));
        YvSum=sum(Yv,2);
        Yv=Yv(YvSum>0,:);
        suffix=sprintf('%s_%s_%s_%s_%s_%s_SPINgreedy', filename,c,g,nb,penalty,type);
    ...
    for k=1:nfold
        ...
        load(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/onto_spin/tmp/Ypred_%s.mat', paramsIn.filestem));
   ...
   %dlmwrite(sprintf('/home/ilievsm1/T_%s.mat',paramsIn.filestem),running_times);
        filename=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/onto_spin/results/Ypred_%s.mat',paramsIn.filestem));
	save(filename,'YpredVal');
 	fname=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/onto_spin/results/T_%s.mat',paramsIn.filestem));
	save(fname,'running_times');
