...
function run_SPINgreedy(c,g,nb,penalty,type,filename)
   ...
   %% change Y from -1 to 0: labeling (0/1), for the sake of base learner svm, will change back in later section
    ...
    for k=1:nfold
        ...
        [rtn,ts_err] = SPINgreedy_test(paramsIn,dataIn,k);
        % collect results
        %load(sprintf('/var/tmp/Ypred_%s.mat', paramsIn.filestem));
        %for triton
        load(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/tmp/Ypred_%s_%i.mat', params.filestem,k));
        Ypred(Itest,:)=Ypred_ts;
        YpredVal(Itest,:)=Ypred_ts_val;
        running_times(k,1) = running_time;
    end

        filename=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/results/Ypred_s%.mat',params.filestem));
	save(filename,YpredVal);
	fname=(sprintf('/triton/ics/work/ilievsm1/for_triton/spin_gdy/results/T_%s.mat',params.filestem));
