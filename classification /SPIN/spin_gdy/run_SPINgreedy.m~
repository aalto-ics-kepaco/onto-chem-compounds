...
function run_SPINgreedy(c,g,nb,penalty,type,filename)
   ...
   %read the input data from onto folder, change the path either to your local
   %directory or the location of onto folder in triton directories
    else if strcmp(filename(1:4),'onto')
        X=dlmread(sprintf('/home/milievsk/Documents/MATLAB/onto/%s.BabPub.phix',filename));
        Y=dlmread(sprintf('/home/milievsk/Documents/MATLAB/onto/%s.Y',filename));
        Yp=Y;
        Y(Y>0)=1;
        E=dlmread(sprintf('/home/milievsk/Documents/MATLAB/onto/%s.e',filename));
        E=E(:,1:2);
        Yv=dlmread(sprintf('/home/milievsk/Documents/MATLAB/onto/%s.Yv',filename));
        YvSum=sum(Yv,2);
        Yv=Yv(YvSum>0,:);
        suffix=sprintf('%s_%s_%s_%s_%s_%s_SPINgreedy', filename,c,g,nb,penalty,type)
     ...
     %  X=dlmread(sprintf('../dblp/dblp.inp.%s.lda.phix',strrep(filename,'_','.')));
        X=dlmread(sprintf('../../MATLAB/onto/%s.babel.phix',filename));
     %  Y=dlmread(sprintf('../dblp/dblp.inp.%s.lda.Y',strrep(filename,'_','.')));
        Y=dlmread(sprintf('../../MATLAB/onto/%s.Y',filename));
        Yp=Y;
        Y(Y>0)=1;
    %   E=dlmread(sprintf('../dblp/dblp.inp.%s.e',strrep(filename,'_','.')));
        E=dlmread(sprintf('../../MATLAB/onto/%s.e',filename));
        E=E(:,1:2);
    %   Yv=dlmread(sprintf('../dblp/dblp.inp.%s.lda.Yv',strrep(filename,'_','.')));
        Yv=dlmread(sprintf('../../MATLAB/onto/%s.Yv',filename));
        YvSum=sum(Yv,2);
        Yv=Yv(YvSum>0,:);
        suffix=sprintf('%s_%s_%s_%s_%s_%s_SPINgreedy', filename,c,g,nb,penalty,type);
   ...
   %% change Y from -1 to 0: labeling (0/1), for the sake of base learner svm, will change back in later section
    ...
    for k=1:nfold
        ...
        %for triton
        %load(sprintf('/triton/ics/work/ilievsm1/tmp/Ypred_%s.mat', params.filestem));
  ...
  % save the results in the results folder, modify the path to your local directory
  ...
  %dlmwrite(sprintf('/home/ilievsm1/T_%s.mat',paramsIn.filestem),running_times);
   
  
   %filename=(sprintf('/triton/ics/work/ilievsm1/results/Ypred_s%.mat',params.filestem));
   %save(filename,YpredVal);
   %filename_Time=(sprintf('/triton/ics/work/ilievsm1/results/T_%s.mat',params.filestem));
   %save(filename_Time,running_times);
